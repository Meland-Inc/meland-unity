...
public static class ResourceComponentExtensions
{
    ...
    /// </summary>
    /// <param name="assetPath">完整路径</param>
    /// <param name="priority">优先级 默认0</param>
    /// <typeparam name="T">需要加载的类型 默认是object 如果是默认的情况可以直接使用不使用泛型的重载</typeparam>
    /// <returns></returns>
    public static async UniTask<T> AwaitLoadAsset<T>(this ResourceComponent resourceComponent, string assetPath, int priority = 0) where T : class
    {
        T resAsset = null;
    ...
    /// <param name="priority">优先级 默认0</param>
    /// <returns></returns>
    public static async UniTask<object> AwaitLoadAsset(this ResourceComponent resourceComponent, string assetPath, int priority = 0)
    {
        return await resourceComponent.AwaitLoadAsset<object>(assetPath, priority);
    }
}